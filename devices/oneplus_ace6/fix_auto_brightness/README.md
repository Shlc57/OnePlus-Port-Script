# 一加 Ace 6 自动亮度适配（PLQ110 / OP6113，面板 1270x2800，165Hz）
...
亮度表与边界参数来自一加 Ace 6 实测（P7 面板，与 Ace 6T 同源）：
...
1. **Display ID 适配**：把底包 `vendor/etc/displayconfig` 的缺失项补入原包
   ...
   选择显示配置（由机型组合入口注入：`4630947185118785939`）：若底包已有同名
...
2. **亮度曲线校准**：使用内置 P7 面板亮度表 `config/display_brightness_config_P_7.xml`
   ...
   边界数值可通过 `ACE6_DEFAULT_NITS` / `ACE6_STANDARD_MAX_NITS` / `ACE6_HBM_MAX_NITS`
   覆盖；面板表缺失时降级为仅 Display ID 适配（Overlay 仍安装）。
3. **启动亮度 Overlay**：`prebuilt/product/overlay/Ace6BootBrightnessOverlay.apk`
   ...
   135 nit 值 0.394047439），供具备 Android SDK（aapt2 + android.jar）的环境重新编译；
...
本模块同时启用**，Ace 6 组合流程 `OPAce6_port.sh` 不应把它与本模块加入同一清单。
...
- 生成的 `screenBrightnessMap` 会覆盖底包 Display ID 配置中的原曲线；重复执行幂等
  （主显示配置经临时文件原子替换）。
- 面板表末端 nits 必须等于 `ACE6_HBM_MAX_NITS`（默认 1776）、首行必须为 `0,0,0`，
...
```bash
PORT_TARGET_DISPLAY_ID=4630947185118785939 bash port_main.sh devices/oneplus_ace6/fix_auto_brightness
```

`PORT_TARGET_DISPLAY_ID` 必须是 uint64 范围内的正十进制 ID。完整 Ace 6 组合流程会提供
该默认值，也可以在调用 `OPAce6_port.sh` 时从外部覆盖。
