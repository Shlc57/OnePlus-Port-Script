...
目标调用是从音频流对象虚表偏移读取 `setParameters` 后执行的 `blr x8`（`00 01 3f d6`），修补后替换为 ARM64 `nop`（`1f 20 03 d5`）。虚表槽位在不同 ROM 版本可能漂移（例如 SM8845 的 `AudioPolicyClientInterface` 新增虚函数使 `setParameters` 从槽位 `0x60` 漂移到 `0x68`），补丁的锚点契约支持按目标 ROM 重新校准槽位偏移。这是无法通过属性或 XML 阻止的库内虚函数调用，因此采用最小二进制定点修补。补丁不修改 32 位库，也不改动其它音量、路由或硬件控制调用。
...
- 每个调用点通过函数语义范围、虚表槽位读取（槽位偏移在不同 ROM 版本可能漂移，当前适配 SM8845 的 `0x68`）、参数寄存器准备以及调用后字符串处理指令共同定位。
...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 features/fuck_audio_appname/test_contract.py
```
