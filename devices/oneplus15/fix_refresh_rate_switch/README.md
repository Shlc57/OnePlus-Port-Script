...
该模块已由 `OP15_port.sh` 在 `common/fix_boot_refresh_rate` 之后调用；也可以单独执行：

```bash
bash port_main.sh devices/oneplus15/fix_refresh_rate_switch
```

需要 Python 3、Java、Apktool、`zip`、`unzip` 和 Android SDK `zipalign`。当前只完成静态/临时 APK 验证，未宣称真实设备生效。
