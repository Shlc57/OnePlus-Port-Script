...
# devices/realme_neo8/fix_refresh_rate_switch —— 与 devices/oneplus15/fix_refresh_rate_switch
# 同一策略：Neo8 沿用一加 15/Ace 6T 的 165Hz 五档屏假设（60/90/120/144/165），DC/PWM 档位策略一致；
# 修补目标（原包机型 XML、MISettings、Settings）均在小米原包侧，与 realme 底包机型无关。
# 差异仅：patcher 使用本目录副本，便于按 Neo8 原包版本微调锚点；启用前需按实机档位重审。

init_port_env "${1:-}"

std_print "修复 Neo8 DC/PWM 与超高刷新率切换"
...
# shellcheck disable=SC2154
misettings_patcher="$port_dir/devices/realme_neo8/fix_refresh_rate_switch/patch_misettings_dc_refresh.sh"
...
# shellcheck disable=SC2154
settings_patcher="$port_dir/devices/realme_neo8/fix_refresh_rate_switch/patch_settings_dc_refresh.sh"
