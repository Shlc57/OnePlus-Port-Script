...
- 应根据任务进展随时建立并更新临时备忘录，记录目标、关键事实、已完成工作、未决事项和验证结果，防止上下文压缩后丢失重要信息。
- 在修复多个相互独立或可安全拆分的问题时，可以启用子智能体并行处理，以缩短处理时间并减轻主智能体的上下文压力。拆分前应明确各子任务的边界，避免多个智能体同时修改同一文件或共享状态；最终由主智能体统一检查、整合并验证结果。
- 长日志读取、或者存在可以同时互补不干扰的多个反编译分析任务时，可以启用子智能体并行处理，以缩短处理时间并减轻主智能体的上下文压力。
