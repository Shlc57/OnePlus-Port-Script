...
# devices/oneplus_ace6t/fix_refresh_rate_switch —— 与 devices/oneplus15/fix_refresh_rate_switch
# 同一策略：Ace 6T 与一加 15 同为 165Hz 五档屏（60/90/120/144/165），DC/PWM 档位策略一致；
# 修补目标（原包机型 XML、MISettings、Settings）均在小米原包侧，与 OPPO 底包机型无关。
# 差异仅：patcher 使用本目录副本，便于按 Ace 6T 原包版本微调锚点。

init_port_env "${1:-}"

std_print "修复 Ace 6T DC/PWM 与超高刷新率切换"
...
# shellcheck disable=SC2154
misettings_patcher="$port_dir/devices/oneplus_ace6t/fix_refresh_rate_switch/patch_misettings_dc_refresh.sh"
...
# shellcheck disable=SC2154
settings_patcher="$port_dir/devices/oneplus_ace6t/fix_refresh_rate_switch/patch_settings_dc_refresh.sh"
