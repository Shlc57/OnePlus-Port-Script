...
策略由 common/fix_vendor_avc 合并到 normal/debug CIL。原 Oplus policy 将多个 reserve 客户端授权给 `oppo_block_device_202504`，但当前移植实际节点使用无法映射的 `oppo_block_device`。本模块因此只补齐日志中出现的 6 个 HAL/daemon 对该具体类型的 `blk_file` 权限；charger 的实测操作还需要 `write`，其余客户端不授予 `write`。Engineer HAL 的 SN 查询实测需要 `open` 与 `read`，且会检索 reserve 挂载根目录，因而额外获得该具体目录类型的 `search`；不授予 `write`。gameopt 仅获得 `domain:dir search` 与 `domain:file read/open/getattr`，用于枚举 `/proc/<pid>` 并读取 `comm`，不获得进程控制权限。reserve 挂载链只补齐实际 `vendor_init` AVC 的 `relabelfrom`、`read`、`search`、`getattr`、`setattr`，标签限定为 reserve 根、system、media、system/config 与 media/log；本次启动新增的 system/config、media/log 文件拒绝分别补齐 `file getattr`。Composer 以基础 `chr_file ioctl` 配合 `0x5500` 精确 ioctl xperm，避免泛化命令范围。qsguard、wlchg、URCC、sensor-calibrate、cameramind 和两个 Oplus vendor app 同样只包含各自 AVC 中的精确 class/权限；cameramind 的域来自 system_ext，bundle 显式验证其 CIL 输入后才启用 Binder `call` 与 zygote socket `getopt`。不能使用 oppo_block_device_202504，因为目标平台没有为这个 Oplus 私有类型提供 mapping；不使用 permissive、dontaudit、通配类型或 rild allow。
