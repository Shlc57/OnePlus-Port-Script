...
- `devices/<device>/<patch>/apply.sh`：只有高度依赖该设备硬编码参数，且参数无法从原包、底包配置或运行时信息可靠获取、只能在该设备上试错确定的补丁，才属于设备专属补丁；例如超声波指纹图标位置。可动态读取或可通用化的适配，不得仅因当前流程面向某设备就归入设备专属，应放在 `common`。
...
- 创建新补丁前，必须先用 `rg` 检查现有 `common` 与 `devices` 补丁的相似语义、目标分区、来源和依赖；如果已有相似语义，应优先合并到现有补丁，不得为了极小改动新建孤立补丁。只有目标边界、生命周期、依赖或执行顺序确实独立时，才允许新建补丁，并说明拆分理由。
