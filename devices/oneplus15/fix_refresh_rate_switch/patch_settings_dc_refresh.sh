...
awk -v entry="$APK_DEX_ENTRY" '$0 == entry { count++ } END { exit count == 1 ? 0 : 1 }' "$ENTRIES_BEFORE" ||
	fail "Settings.apk 中 $APK_DEX_ENTRY 数量不是 1"
archive_non_target_snapshot "$APK_PATH" "$APK_DEX_ENTRY" "$NON_TARGET_BEFORE"
...
archive_non_target_snapshot "$PATCHED_APK" "$APK_DEX_ENTRY" "$NON_TARGET_AFTER"
