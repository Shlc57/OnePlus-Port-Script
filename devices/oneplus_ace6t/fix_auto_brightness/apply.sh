...
std_print "修复一加 Ace 6T 自动亮度"
std_print "校准启动默认亮度与物理边界：默认 135 nit，普通亮度 782 nit，HBM 1776 nit（P7 面板实测）"
std_print "边界数值可通过 ACE6T_DEFAULT_NITS / ACE6T_STANDARD_MAX_NITS / ACE6T_HBM_MAX_NITS 覆盖"
...
boot_brightness_overlay="$patcher_dir/prebuilt/product/overlay/Ace6TBootBrightnessOverlay.apk"
boot_brightness_overlay_checksums="$patcher_dir/config/boot_brightness_overlay.sha256"
boot_brightness_overlay_target="$project_dir/product/overlay/Ace6TBootBrightnessOverlay.apk"
...
ACE6T_DEFAULT_NITS="${ACE6T_DEFAULT_NITS:-135}"
ACE6T_STANDARD_MAX_NITS="${ACE6T_STANDARD_MAX_NITS:-782}"
# P7 面板亮度表实测末端 1776 nit（display_brightness_config_P_7.xml）；高于旧文档预估的 1800 需以实表为准。
ACE6T_HBM_MAX_NITS="${ACE6T_HBM_MAX_NITS:-1776}"
export ACE6T_DEFAULT_NITS ACE6T_STANDARD_MAX_NITS ACE6T_HBM_MAX_NITS
...
if [[ -f "$panel_brightness_config" ]]; then
	...
	warn_print "未找到 Ace 6T 面板亮度表，跳过亮度曲线校准（仅执行 Display ID 适配）"
	warn_print "如需完整校准，请将 Ace 6T 面板 P7 亮度表放入：${panel_brightness_config#"$patcher_dir"/}"
fi
...
if ! command -v python3 >/dev/null 2>&1; then
	err_print "缺少 Python 3，无法生成 Ace 6T 高亮度映射"
	exit 1
fi
...
default_nits = float(os.environ.get("ACE6T_DEFAULT_NITS", "135"))
standard_max_nits = float(os.environ.get("ACE6T_STANDARD_MAX_NITS", "782"))
hbm_max_nits = float(os.environ.get("ACE6T_HBM_MAX_NITS", "1776"))
...
PY
	...
	std_print "✅ 已生成 $ACE6T_DEFAULT_NITS/$ACE6T_STANDARD_MAX_NITS/$ACE6T_HBM_MAX_NITS nit 边界校准映射：$brightness_summary"
else
	warn_print "跳过亮度曲线校准：未提供 Ace 6T 面板亮度表 ${panel_brightness_config#"$patcher_dir"/}"
fi
...
std_print "✅ 已加入仅覆盖启动默认亮度的 135 nit 单资源 Overlay（Ace6TBootBrightnessOverlay）"
